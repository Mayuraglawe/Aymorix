...
export function AymorixLogo({ size = 40, className = '', variant = 'full', color = '#0D9488' }: AymorixLogoProps) {
  if (variant === 'icon') {
    return (
      <svg
        width={size}
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <span className="font-bold tracking-tight" style={{ fontSize: size * 0.6, color }}>
        Aymorix
      </span>
    </div>
  );
